...
class TCPDF {
	...
	public function Footer() {
		...
		if (empty($this->pagegroups)) {
			...
			$pagenumtxt = 'GF COBRANZAS JURIDICAS - Creado en  ' .$timestamp.'   '.$this->l['w_page'].' '.$this->getAliasNumPage().' / '.$this->getAliasNbPages();

		} else {

			$pagenumtxt = ' GF COBRANZAS JURIDICAS -  Creado en' .$timestamp.'   '. $this->l['w_page'].' '.$this->getPageNumGroupAlias().' / '.$this->getPageGroupAlias();
		...
		if ($this->getRTL()) {
			...
			$this->SetX($this->original_lMargin);
			$this->Cell(1, 1, $this->getAliasRightShift().$pagenumtxt, 'T', 0, 'L');
		}
	}
